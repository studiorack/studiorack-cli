...
const VALIDATOR_PATH = path.join(__dirname.substring(0, __dirname.lastIndexOf('lib')), 'bin', 'validator');
...
async function install() {
  ...
  if (!file.directoryExists(VALIDATOR_PATH)) {
    ...
    console.log('substring', __dirname.substring(0, __dirname.lastIndexOf('lib')));
    console.log('VALIDATOR_PATH', VALIDATOR_PATH);
    file.extractZip(data, VALIDATOR_PATH);
    file.makeExecutable(`${VALIDATOR_PATH}/validator`);
    return true;
  }
  return false;
}
