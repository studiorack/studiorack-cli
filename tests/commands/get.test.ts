...
test('Get package', async () => {
  ...
  expect(await cli(RegistryType.Plugins, 'get', 'surge-synthesizer/surge')).toMatchSnapshot();
  expect(await cli(RegistryType.Plugins, 'get', 'surge-synthesizer/surge@1.3.1')).toMatchSnapshot();
  expect(await cli(RegistryType.Plugins, 'get', 'surge-synthesizer/surge@0.0.0')).toMatchSnapshot();
});
